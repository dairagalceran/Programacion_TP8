...
public class ConjuntoPieza extends ElementoAdmin {
    ...
    protected ConjuntoPieza instanciar(){
        ConjuntoPieza copia = new ConjuntoPieza(this.getMarca(), this.getPorcentajeDescuento());
        return copia;
    }

    @Override
    public ElementoAdmin getCopia(){
        ConjuntoPieza copia = this.instanciar();
        for(int i =0; i< comboPaquete.size();i++){
            copia.addPiezasAlCombo(comboPaquete.get(i).getCopia());
        }
        return copia;
    }
